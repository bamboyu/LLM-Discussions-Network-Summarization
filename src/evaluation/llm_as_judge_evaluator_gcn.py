...
def run_evaluator():
    # Đường dẫn file tóm tắt của GAT (Model gốc)
    gat_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../data/processed/final_summaries_gpt4o.json'))
    # Đường dẫn file tóm tắt của GCN
    gcn_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../data/processed/gcn_summaries_gpt4o.json'))
    # Đổi tên file kết quả để không đè lên file cũ
    output_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../data/processed/gcn_vs_gat_evaluation_results.json'))

    print("Đang nạp dữ liệu từ hai hệ thống (GAT và GCN)...")
    try:
    ...
    common_ids = list(set(gat_data.keys()).intersection(set(gcn_data.keys())))
    ...
    print(f"Bắt đầu chấm điểm {len(common_ids)} bài đối chứng GAT vs GCN...")
    ...
    for thread_id in tqdm(common_ids):
        ...
        # Đảo ngẫu nhiên vị trí nhưng cố định theo thread để tái lập kết quả
        if is_gat_a:
            summary_a, summary_b = sum_gat, sum_gcn
        else:
            summary_a, summary_b = sum_gcn, sum_gat
        ...
        while not success and retries < max_retries:
            try:
                ...
                # Ánh xạ lại điểm số từ A/B về đúng GAT/GCN
                gat_score = eval_result["Score_A"] if is_gat_a else eval_result["Score_B"]
                gcn_score = eval_result["Score_B"] if is_gat_a else eval_result["Score_A"]
                
                # Xác định Winner thật sự
                if eval_result["Winner"] == "Tie":
                    real_winner = "Tie"
                elif (eval_result["Winner"] == "A" and is_gat_a) or (eval_result["Winner"] == "B" and not is_gat_a):
                    real_winner = "GAT"
                else:
                    real_winner = "GCN"
    ...
    print(f"\n[XONG] Kết quả chấm điểm GAT vs GCN đã lưu tại: {output_path}")
