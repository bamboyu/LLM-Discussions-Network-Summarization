...
def run_evaluator():
    ...
    output_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../data/processed/llm_eval_gat_vs_gcn.json'))
    ...
    print(f"Bắt đầu chấm điểm {len(common_ids)} bài (GAT vs GCN)...")
    ...
    for thread_id in tqdm(common_ids):
        ...
        while not success and retries < max_retries:
            try:
                ...
                gcn_scores = scores_b if is_gat_a else scores_a

                gat_total = round(sum(float(v) for v in gat_scores.values()) / len(gat_scores), 2)
                gcn_total = round(sum(float(v) for v in gcn_scores.values()) / len(gcn_scores), 2)
                ...
                results.append({
                    ...
                    "GCN": {**gcn_scores, "Total": gcn_total},
                    "Winner": real_winner,
