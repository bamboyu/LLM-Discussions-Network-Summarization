...
from torch_geometric.nn import GCNConv # Đã đổi từ GATConv sang GCNConv

class RedditGCN(torch.nn.Module):
    # Đã bỏ tham số heads vì GCN không dùng cơ chế attention
    def __init__(self, in_channels, hidden_channels=64, out_channels=1):
        super(RedditGCN, self).__init__()
        # GCNConv không có tham số heads, concat, hay dropout bên trong hàm khởi tạo
...
def train_model():
    ...
    # Cập nhật đường dẫn file save để không đè lên model GAT cũ
    model_save_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../data/processed/gcn_model_final.pth'))
    ...
    # Khởi tạo model GCN
    model = RedditGCN(in_channels=in_channels).to(device)
    ...
    # Khởi tạo lại model GCN
    model = RedditGCN(in_channels=best_checkpoint['in_channels']).to(device)
