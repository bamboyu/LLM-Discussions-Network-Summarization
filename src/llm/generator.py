...
def generate_summary(prompt_text):
    ...
    response = client.chat.completions.create(
        ...
        temperature=0.3,
        max_tokens=200  # Giới hạn output ~800-900 ký tự, input/thinking không bị ảnh hưởng
    )
...
def extract_with_one_parent(selected_comments, all_comment_by_id):
    """
    Với mỗi comment được chọn (GAT output), lấy thêm đúng 1 comment cha trực tiếp.
    Deduplicate để tránh trùng khi 2 comment cùng cha.
    Giữ nguyên toàn bộ selected_comments, chỉ bổ sung cha còn thiếu.
    """
    enriched = {c['id']: c for c in selected_comments}  # Dùng dict để deduplicate tự động
...
def format_comments(comments, all_comment_by_id):
    ...
    # Dùng all_comment_by_id để tính depth chính xác (kể cả cha không nằm trong batch)
    memo = {}
    ...
    for c in sorted_comments:
        ...
        # Ưu tiên tìm author cha trong batch hiện tại, fallback sang all_comment_by_id
...
def run_generator():
    ...
    print("Loading extracted contexts from GAT...")
    try:
